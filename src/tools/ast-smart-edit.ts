...
const astSmartEditSchema = z.object({
    ...
    selector: z.string().optional().describe('CSS-like selector for AST nodes (e.g., "FunctionDeclaration", "CallExpression[callee.name=console]")'),
    transform: z.enum(['add-import', 'remove-import', 'rename-function', 'rename-variable', 'extract-function', 'inline-variable']).optional()
...
- Pattern replace: { pattern: 'console.log($ARG)', replacement: 'logger.debug($ARG)' }`,
    ...
    execute: async ({ path, pattern, replacement, selector, transform, targetName, newName, importSpec }) => {
        try {
...
function addImport(content: string, root: any, importSpec: { module: string; names: string[]; isDefault?: boolean }): string {
...
function renameFunction(content: string, root: any, oldName: string, newName: string): { content: string; count: number } {
    let modifiedContent = content;
    let count = 0;
...
function renameVariable(content: string, root: any, oldName: string, newName: string): { content: string; count: number } {
    let modifiedContent = content;
    let count = 0;
...
function patternReplace(content: string, root: any, pattern: string, replacement: string): { content: string; count: number } {
    let modifiedContent = content;
    let count = 0;
