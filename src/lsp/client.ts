...
export class LSPClient {
  ...
  async initialize(): Promise<void> {
    ...
    (this.connection as any).onNotification((method: string, params: any) => {
...
window: {
    ...
    this.connection.onRequest('window/workDoneProgress/create', (params: any) => {
   ...
   */
  async waitForDiagnostics(filePath: string, timeoutMs: number = 5000, waitForChange: boolean = false): Promise<Diagnostic[]> {
    if (!this.connection) return [];
        ...
        if (processAlive) {
          await Promise.race([
            ...
            new Promise((_, reject) => 
              setTimeout(() => reject(new Error('Shutdown request timed out')), 1000)
            )
          ]);
