...
class LSPWrapper {
   ...
   */
  async getClients(filePath: string, cwd: string = this.cwd): Promise<LSPClient[]> {
   ...
   */
  async touchFile(filePath: string, waitForDiagnostics: boolean = false, cwd: string = this.cwd): Promise<void> {
  ...
  async diagnostics(filePath: string, cwd: string = this.cwd): Promise<any[]> {
    ...
    for (const client of clients) {
      ...
      allDiagnostics.push(...diagnostics.map(d => ({
        severity: d.severity,
        message: d.message,
        range: d.range,
        source: d.source || 'lsp'
   ...
   */
  async hover(filePath: string, line: number, character: number, cwd: string = this.cwd): Promise<any> {
    const clients = await this.getClients(filePath, cwd);
...
export const getClients = (filePath: string, cwd?: string) => lsp.getClients(filePath, cwd);
export const touchFile = (filePath: string, waitForDiagnostics?: boolean, cwd?: string) => 
  lsp.touchFile(filePath, waitForDiagnostics, cwd);
export const diagnostics = (filePath: string, cwd?: string) => lsp.diagnostics(filePath, cwd);
export const hover = (filePath: string, line: number, character: number, cwd?: string) => 
