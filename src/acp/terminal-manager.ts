...
export class TerminalManager extends EventEmitter {
  ...
  async waitForExit(terminalId: string, timeoutMs?: number): Promise<TerminalOutput> {
    const session = this.terminals.get(terminalId)
    if (!session) {
  ...
  async killTerminal(terminalId: string, signal: NodeJS.Signals = 'SIGTERM'): Promise<boolean> {
    const session = this.terminals.get(terminalId)
    if (!session) {
      return false
    }
  ...
  private handleExit(terminalId: string, code: number | null, signal: NodeJS.Signals | null): void {
    const session = this.terminals.get(terminalId)
    if (!session) return
