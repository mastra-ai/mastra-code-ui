...
export async function loadCustomCommands(projectDir?: string): Promise<SlashCommandMetadata[]> {
  ...
  // 2. Load from mastra user directory ~/.mastra/commands
  if (homeDir) {
    const mastraUserDir = path.join(homeDir, '.mastra', 'commands');
  ...
  // 4. Load from mastra project directory .mastra/commands (highest priority)
  if (projectDir) {
    const mastraProjectDir = path.join(projectDir, '.mastra', 'commands');
...
export function getProjectCommandsDir(projectDir: string): string {
  return path.join(projectDir, '.mastra', 'commands');
}
