...
import {
	...
	GitForkIcon,
	PullRequestSmallIcon,
	SidebarLeftClosedIcon,
	SidebarRightClosedIcon,
...
export function App() {
	...
	const isRightSidebarOpen =
		rightSidebarVisible && project.enrichedProjects.length > 0

	useEffect(() => {
		const viewport = tabScrollRef.current
		const activeTab = viewport?.querySelector<HTMLElement>("[data-tab-active]")
		activeTab?.scrollIntoView({ block: "nearest", inline: "nearest" })
	}, [tabScrollRef, tabs.activeTab, tabs.openFiles, tabs.openThreadTabs])
	...
	const handleNewThread = useCallback(async () => {
		const isDraftThread = (thread?: ThreadInfo) => {
			const title = thread?.title?.trim()
			return !title || title === "New Thread" || title === "Untitled"
		}

		const activeThreadId = tabs.activeTab.startsWith("thread:")
			? tabs.activeTab.slice(7)
			: null
		...
		if (
			...
			isDraftThread(activeThread)
		) {
			setEditorFocusToken((token) => token + 1)
			return
		}

		if (tabs.openThreadTabs.length === 0 && chat.messages.length === 0) {
			tabs.setActiveTab("chat")
			setEditorFocusToken((token) => token + 1)
			return
		}

		const existingDraftThreadId = tabs.openThreadTabs.find((threadId) => {
		...
		if (existingDraftThreadId) {
			...
			tabs.setActiveTab(`thread:${existingDraftThreadId}`)
			setEditorFocusToken((token) => token + 1)
			return
		}

		const createThread = async (activate: boolean) => {
			...
			if (thread?.id) {
				...
				if (activate) tabs.setActiveTab(`thread:${thread.id}`)
			}
			return thread
		}
	...
	const handleQuestionResponse = useCallback(
		async (questionId: string, answer: QuestionAnswer) => {
			dispatch({ type: "QUESTION_RESPONSE", questionId, answer })
			await dialogs.handleQuestionResponse(questionId, answer)
		},
		[dialogs.handleQuestionResponse, dispatch],
	)
	...
	const handleDeleteThread = useCallback(
		async (threadId: string) => {
			const result = (await window.api.invoke({
				type: "deleteThread",
				threadId,
			})) as DeleteThreadResult | undefined
			const isDeletingCurrentThread = currentThreadId === threadId
			const nextThreadId = isDeletingCurrentThread
				? (result?.currentThreadId ?? null)
				: null

			tabs.setOpenThreadTabs((prev) => {
				const next = prev.filter((id) => id !== threadId)
				if (nextThreadId && !next.includes(nextThreadId)) {
					next.push(nextThreadId)
				}
				return next
			})
			tabs.setActiveTab((activeTab) => {
				if (activeTab !== `thread:${threadId}`) return activeTab
				return nextThreadId ? `thread:${nextThreadId}` : "chat"
			})
	...
	const handleSetThinkingLevel = useCallback(async (level: string) => {
		setThinkingLevel(level)
		await window.api.invoke({ type: "setThinkingLevel", level })
	}, [])
		...
		>
			...
			>
				...
				>
					{!sidebarVisible && (
						<button
							...
							title="Show sidebar"
						>
							<SidebarLeftClosedIcon width={16} height={16} />
						</button>
					)}

					<div
						ref={tabScrollRef}
						className="titlebar-no-drag session-tabs-scroll"
					>
					...
					{!isRightSidebarOpen && project.enrichedProjects.length > 0 && (
						<button
							className="titlebar-no-drag"
							...
							title="Show Explorer (Cmd+Shift+E)"
						>
							<SidebarRightClosedIcon width={16} height={16} />
						</button>
					)}
				</div>
				  ...
				  (tabs.activeTab === "chat" ||
					...
					>
						...
						<div style={{ textAlign: "center", maxWidth: 420 }}>
							...
							>
								Open a folder or clone a repo. MastraCode works from there.
							</div>
						</div>
						<div style={{ display: "flex", gap: 12 }}>
							...
							>
								<FolderOpenSimpleIcon width="18" height="18" />
								Open folder
							</button>
							<button
					...
					<>
						...
						>
							<ChatView
								...
								pendingQuestion={dialogs.pendingQuestion}
				...
				<StatusBar
					...
					onSelectModel={dialogs.handleSwitchModel}
			...
			<RightSidebar
				...
				onToggleVisible={() => setRightSidebarVisible(false)}
			/>
