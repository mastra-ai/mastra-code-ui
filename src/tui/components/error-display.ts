...
export class ErrorDisplayComponent extends Container {
    ...
    private build(): void {
        ...
        if (info.name && info.name !== "Error") {
            ...
            typeLine.addChild(new Text(
                errorBg(` ${bold(fg("error", info.name))} `), 
                0, 0
            ))
        ...
        if (info.file && info.line) {
            ...
            errorContainer.addChild(new Text(
                fg("muted", `  at ${location}`), 
                0, 0
            ))
        }
        ...
        if (this.options.showStack && info.stack) {
            this.addChild(new Spacer(1))
            this.addChild(new CollapsibleStackTrace(
                info.stack,
