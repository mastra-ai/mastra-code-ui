...
import { ModelSelectorComponent, type ModelItem } from "./components/model-selector.js"
...
import { OMProgressComponent, type OMProgressState, formatObservationStatus, formatReflectionStatus } from "./components/om-progress.js"
...
import { getEditorTheme, getMarkdownTheme, getTheme, fg, bold, getContrastText } from "./theme.js"
...
export class MastraTUI {
  ...
  private modelAuthStatus: { hasAuth: boolean; apiKeyEnvVar?: string } = { hasAuth: true }
   ...
   */
  private async promptYesNo(question: string, defaultYes: boolean): Promise<boolean> {
  ...
  private buildLayout(): void {
    ...
    this.ui.addChild(new Text(`${logo}\n${instructions}`, 1, 0))
    this.ui.addChild(new Spacer(1))
  ...
  private updateStatusLine(): void {
    ...
    if (modes.length > 1) {
      ...
      if (currentMode?.color) {
        ...
        modeBadge = chalk.bgHex(currentMode.color).hex(textColor).bold(` ${modeName} `) + " "
      } else {
    ...
    if (!this.modelAuthStatus.hasAuth) {
      ...
      agentParts.push(modelId + fg("error", " ✗") + fg("muted", envVar ? ` (${envVar})` : " (no key)"))
    } else {
  ...
  private async handleEvent(event: HarnessEvent): Promise<void> {
    switch (event.type) {
      ...
      case "tool_approval_required":
        await this.handleToolApprovalRequired(event.toolCallId, event.toolName, event.args)
        break
      ...
      case "om_observation_end":
        this.handleOMObservationEnd(event.cycleId, event.durationMs, event.tokensObserved, event.observationTokens)
        break
      ...
      case "om_reflection_end":
        this.handleOMReflectionEnd(event.cycleId, event.durationMs, event.compressedTokens)
        break
  ...
  private syncOMThresholdsFromHarness(): void {
    ...
    this.omProgress.thresholdPercent = obsThreshold > 0
      ? (this.omProgress.pendingTokens / obsThreshold) * 100
      : 0
    this.omProgress.reflectionThreshold = refThreshold
    this.omProgress.reflectionThresholdPercent = refThreshold > 0
  ...
  private handleOMProgress(event: {
    ...
    if (this.omProgress.status === "observing" || this.omProgress.status === "reflecting") {
      ...
      this.omProgress.reflectionThreshold = event.reflectionThreshold ?? this.omProgress.reflectionThreshold
      this.omProgress.reflectionThresholdPercent = event.reflectionThresholdPercent ?? this.omProgress.reflectionThresholdPercent
      this.updateStatusLine()
      return
    }
  ...
  private handleOMObservationStart(cycleId: string, tokensToObserve: number): void {
    ...
    const tokens = tokensToObserve > 0 ? ` ~${this.formatTokensShort(tokensToObserve)} tokens` : ""
    this.updateLoaderText(`Observing${tokens}...`)
    this.ui.requestRender()
    this.updateStatusLine()
  }

  private handleOMObservationEnd(_cycleId: string, durationMs: number, tokensObserved: number, observationTokens: number): void {
    ...
    this.chatContainer.addChild(new OMMarkerComponent({
  ...
  private handleOMReflectionStart(cycleId: string, tokensToReflect: number): void {
    ...
    this.omProgress.cycleId = cycleId
    this.omProgress.startTime = Date.now()
    const tokens = tokensToReflect > 0 ? ` ~${this.formatTokensShort(tokensToReflect)} tokens` : ""
    this.updateLoaderText(`Reflecting${tokens}...`)
    this.ui.requestRender()
    this.updateStatusLine()
  }

  private handleOMReflectionEnd(_cycleId: string, durationMs: number, compressedTokens: number): void {
    ...
    this.omProgress.reflectionThresholdPercent = this.omProgress.reflectionThreshold > 0
  ...
  private handleAgentAborted(): void {
    ...
    this.statusContainer.addChild(new Text(fg("warning", "⚠ Interrupted"), 0, 0))
    this.ui.requestRender()
  ...
  private handleMessageUpdate(message: HarnessMessage): void {
    ...
    // Check for new tool calls (use seenToolCallIds to avoid duplicates after tool_end deletes from pendingTools)
    for (const content of message.content) {
      if (content.type === "tool_call") {
        if (!this.seenToolCallIds.has(content.id)) {
          ...
          this.allToolComponents.push(component)  // Track for expand/collapse

          // Create a new streaming component for text that follows the tool calls
          this.streamingComponent = new AssistantMessageComponent(
            undefined,
            this.hideThinkingBlock,
            getMarkdownTheme()
    ...
    // Update the streaming component with only the trailing text/thinking parts
    // (everything after the last tool_call/tool_result)
  ...
  private handleMessageEnd(message: HarnessMessage): void {
    ...
    if (this.streamingComponent && message.role === "assistant") {
      this.streamingMessage = message
      // Only render trailing parts (after last tool call) in the current streaming component
      const trailingParts = this.getTrailingContentParts(message)
      this.streamingComponent.updateContent({
        ...message,
        content: trailingParts,
      ...
      const dialog = new ToolApprovalDialogComponent({
        ...
        onApprove: async () => {
          ...
          try {
            ...
            this.showError(error instanceof Error ? error.message : "Failed to approve tool")
          }
          resolve()
        },
        onDecline: async () => {
          ...
          try {
            ...
            this.showError(error instanceof Error ? error.message : "Failed to decline tool")
          }
          resolve()
        },
      })
      ...
      const settings = new OMSettingsComponent(
        config,
        {
          ...
          onObservationThresholdChange: (value) => {
            ...
            this.omProgress.thresholdPercent = value > 0
              ? (this.omProgress.pendingTokens / value) * 100
              : 0
            this.updateStatusLine()
          },
          onReflectionThresholdChange: (value) => {
            ...
            this.omProgress.reflectionThresholdPercent = value > 0
              ? (this.omProgress.observationTokens / value) * 100
              : 0
            this.updateStatusLine()
          },
          onClose: () => {
    ...
    return new Promise<void>((resolve) => {
      const settings = new ThinkingSettingsComponent(currentLevel, {
        onLevelChange: async (level) => {
          ...
          const label = level === "off" ? "Off" : level.charAt(0).toUpperCase() + level.slice(1)
...
Keyboard shortcuts:
          ...
          if (content.type === "text" || content.type === "thinking") {
            ...
            if (toolResult && toolResult.type === "tool_result") {
              toolComponent.updateResult(
                {
                  content: [{ type: "text", text: this.formatToolResult(toolResult.result) }],
   ...
   */
  private showFormattedError(event: { error: Error; errorType?: string; retryable?: boolean; retryDelay?: number }): void {
    ...
    this.chatContainer.addChild(
      new Text(fg("error", errorText), 1, 0)
    )
